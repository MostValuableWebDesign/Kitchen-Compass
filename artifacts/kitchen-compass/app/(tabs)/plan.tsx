...
export default function PlanScreen() {
  ...
  const selectedRecipe = selectedMeal ? availableRecipes.find((item) => item.id === selectedMeal.recipeId && (!selectedMeal.recipeVersion || recipeVersion(item) === selectedMeal.recipeVersion)) : undefined;
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background, paddingTop: insets.top + 12 }]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {days.map((day) => <View key={day} style={[styles.dayCard, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={styles.dayHeader}><Text style={[styles.dayName, { color: colors.foreground }]}>{day}</Text><Text style={[styles.dayMeta, { color: colors.mutedForeground }]}>{day === new Date().toLocaleDateString('en-US', { weekday: 'long' }) ? 'TODAY' : ''}</Text></View>{mealTypes.map((meal) => { const planned = getMeal(day, meal); const recipe = planned?.leftoverId ? availableRecipes.find((item) => item.id === planned.recipeId) : availableRecipes.find((item) => item.id === planned?.recipeId && (!planned?.recipeVersion || recipeVersion(item) === planned.recipeVersion)); const needsConfirmation = Boolean(planned?.needsConfirmation || reservations.some((reservation) => reservation.plannedMealId === planned?.id && !reservation.quantityKnown)); return <Pressable testID={`meal-${day}-${meal}`} key={meal} onPress={() => openSlot(day, meal)} style={({ pressed }) => [styles.mealSlot, { borderTopColor: colors.border }, pressed && styles.pressed]}><Text style={[styles.mealType, { color: colors.mutedForeground }]}>{meal}</Text><View style={{ flex: 1 }}><Text style={[styles.mealRecipe, { color: recipe ? colors.foreground : colors.mutedForeground }]}>{planned?.leftoverId ? `Leftovers · ${recipe?.title ?? 'meal'}` : recipe?.title ?? 'Tap to add'}</Text>{recipe ? <Text style={[styles.mealTime, { color: colors.mutedForeground }]}>{recipe.prep + recipe.cook} min · {planned?.servings ?? preferences.servings} servings{needsConfirmation ? ' · Confirm quantities' : ''}</Text> : null}</View><Feather name={recipe ? 'edit-3' : 'plus'} size={15} color={recipe ? colors.primary : colors.mutedForeground} /></Pressable>; })}</View>)}
